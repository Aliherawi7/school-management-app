...
function Configuration() {
  ...
  const handleDeleteSewModel = (type: string, index: number) => {
    const temp = { ...config };

    temp?.sewModels[type]?.splice(index, 1);
    updateSystemConfig(temp)
      .then(res => {
        setconfig(temp);
              ...
              {config?.sewModels[key].map((name, index) => {
                return <div className='model_box' key={name + index}>
                  ...
                  <span className='delete_btn'
                    onClick={() => handleDeleteSewModel(key, index)}
                  ><i className={ICONS.trash}></i></span>
                </div>
              })}
            </div>
